...
export default function YogaPoseEstimation() {
  ...
  const analyzePose = (landmarks) => {
    ...
    if (leftHip && leftKnee && leftAnkle && rightHip && rightKnee && rightAnkle) {
      if (isTreePose(leftHip, leftKnee, leftAnkle, rightHip, rightKnee, rightAnkle)) {
        if (!isInPose) {
          isInPose = true;
          count++;
          setPoseCount(count);
  ...
  const isTreePose = (leftHip, leftKnee, leftAnkle, rightHip, rightKnee, rightAnkle) => {
    ...
    return leftLegStraight && rightLegBent;
  };
  ...
  return (
    <div className="min-h-screen bg-black text-white flex flex-col items-center p-6">
      <h1 className="text-3xl font-bold flex items-center gap-2">
        <span className="text-red-500">AI</span> Tree Pose Counter
      </h1>
